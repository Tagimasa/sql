...
FROM customer_purchases ORDER by customer_id) x
...
SELECT 
product_name, substr(product_name, CASE WHEN instr(product_name, '-')=0 THEN NULL ELSE instr(product_name, '-')+2 END) description
FROM product
...
UNION SELECT market_date, min(daily_sales) FROM cte
